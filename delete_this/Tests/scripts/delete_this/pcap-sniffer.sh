...
OPT_COMPONENT_OPERATION="--component";
...
main()
{
	...
	if [[ $arg1 == ${OPT_TIME_CAP} ]]
		then
		#cd scripts/; exec;
		#local exName=${EXPERIMENT_NAME};
		#local pcap_file="../pcaps/"${PCAP_FILE};
	...
	elif [[ $arg1 == ${OPT_COMPONENT_OPERATION} ]]
		then

		##qualification
		#	local name1="diurnalFirewallCapture";
		#	local coment1="LAN diurnal capture (24 hours) on the Firewall";
		#	local pcap_file="../pcaps/lan_diurnalFirewallCapture.pcap";
		#
		...
		store_fileToDatabase $exName $coment1;
		parser_toMatlab $exName
		./plotTraceData.m ${MATLAB_DATA}$exName".txt" ${exName};
...
parser_toMatlab()
{
	...
	if [[ $experimentID == "" ]]
		...
		echo "${experiment_name} ${experimentID}"  > ${MATLAB_DATA}${experiment_name}.txt;
		echo "frame__number flowID frame__len frame__time_delta frame__time_relative" >> ${MATLAB_DATA}${experiment_name}.txt;

		sqlite3  -separator ' ' ${TRACE_DATABASE} "select frame__number, flowID, frame__len, frame__time_delta, frame__time_relative from TraceCapture where experimentID='${experimentID}'" >>  ${MATLAB_DATA}${experiment_name}.txt;
