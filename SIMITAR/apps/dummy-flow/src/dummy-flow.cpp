...
int main(int argc, char** argv)
{
	try
	{
		...
		TCLAP::ValueArg<std::string> mode_arg("m", "mode", "Operation mode, as a client or a server",
				true, "", "string");
		TCLAP::ValueArg<std::string> in_arg("x", "xml", "xml", true, "Compact Trace Descriptor XML. They are salved at data/xml/ directory.",
				"string");
		TCLAP::ValueArg<std::string> dst_list_arg("d", "dst-list-ip", "Text file in CSV format containing a list of desired destinations of IP and MAC adresses, used as servers, in the client mode. These destinations are addressed to the flows in a ring-oriented way.",
				false, "", "string");
		TCLAP::ValueArg<std::string> dst_server_arg("p", "dst-ip", "Instead of a textfile, defines a single server destination IP address in the command line.", false,
				"", "string");
		TCLAP::ValueArg<std::string> eth_arg("e", "ether", "Ethernet interface to send the traffic, as provided by the command `ifconfig`.", false, "",
				"string");
		...
		NetworkTrace traceCdt = NetworkTrace(input_xml, "dummy");
