...
The Flow Generator handles the data on the \textit{Compact Trace Descriptor} file, and use to retrieve parameters for traffic generation. It crafts and controls each flow in a separated thread. To valitate its operation we implemented using Iperf traffic generation\cite{web-iperf} and Libtins C++ API.  To simplify the expansion of this component, we use the design pattern factory. If the user wants to introduce support for a new traffic generator, he just has to expand the class \texttt{DummyFlow}. The new class must not have any new attribute.  Support for this class must be given in the \texttt{NetworkFlowFactory} class.
...
At the flow-level, SIMITAR controls each flow using the algorithm at figure~\ref{alg-simplified-harpoon}. This algorithm handles our model defined at the figure~\ref{fig:modified-harpoon-model}. This procedure is independet of the traffic generator being used.
...
At the packet level, the  \texttt{DummyFlow} derived class must implement  a new \texttt{flowGenerate()} method. Here, the job of our tool is to configure the underlying engine, to send a \textit{file}, as defined in the figure ~\ref{fig:modified-harpoon-model}. This method must use the available parameters, atributes and \textit{getters} to configure and generate a thread-safe traffic.  The implementation of this level should follow the flowchart presented in the figure ~\ref{fig:alg-traffic-engine-config}.
...
void DitgFlow::flowGenerate(const counter& flowId, const time_sec& onTime, const uint& npackets, const uint& nbytes,  const string& netInterface)
{
	...
	for(uint i = 0;;i++)
	{	
		StochasticModelFit idtModel = getInterDepertureTimeModel(i);
	...
	int rc = DITGsend(localhost.c_str(), command.c_str()); 
